...
unless both configured URLs are loopback addresses. When enabled, the login
page shows a development-login button and the supplied local commands bind the
application to localhost. Set `FRONTEND_URL` to the exact browser origin; it is
also the backend CORS allowlist.
